...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList("Cooking failed to start. Make sure you have the correct ingredients in the same directory as HeisenbergHex-Qt.", ERROR, NULL);
    }
}
...
{
    if (getMiningType() == ClientModel::SoloMining)
        reportToList("Solo cooking stopped.", ERROR, NULL);
    else
        reportToList("Cook exited.", ERROR, NULL);
    ui->list->addItem("");
    minerActive = false;
    resetMiningButton();
...
void MiningPage::minerStarted()
{
    if (!minerActive)
        if (getMiningType() == ClientModel::SoloMining)
            reportToList("Solo cooking started.", ERROR, NULL);
        else
            reportToList("Cook started. You might not see any output for a few minutes.", STARTED, NULL);
    minerActive = true;
    resetMiningButton();
...
{
    ui->startButton->setText(minerActive ? "Stop Cooking" : "Start Cooking");
